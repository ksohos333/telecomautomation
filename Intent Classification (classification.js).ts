...
export default function startImapListener(onTicket: (ticket: Ticket) => void): Imap {
  ...
  const imap = new Imap({
    user: process.env.EMAIL_USER || 'konstantinos.sohos@gmail.com',  // Use empty string as fallback
    password: process.env.EMAIL_APP_PASSWORD || 'vzkl klav hpqf xnbv',  // Use empty string as fallback
